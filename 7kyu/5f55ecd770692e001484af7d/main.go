...
func main() {
	fmt.Println(Mirror([]int{}))                         // {}))
	fmt.Println(Mirror([]int{1}))                        // {1}))
	fmt.Println(Mirror([]int{2, 1}))                     // {1, 2, 1}))
	fmt.Println(Mirror([]int{1, 3, 2}))                  // {1, 2, 3, 2, 1}))
	fmt.Println(Mirror([]int{-8, 42, 18, 0, -16}))       // {-16, -8, 0, 18, 42, 18, 0, -8, -16}))
	fmt.Println(Mirror([]int{-5, 10, 8, 10, 2, -3, 10})) // {-5, -3, 2, 8, 10, 10, 10, 10, 10, 8, 2, -3, -5}))
}

func Mirror(data []int) []int {
	var response = make([]int, len(data), len(data)*2)
	copy(response, data)
	sort.Ints(response)

	for i := len(response) - 2; i >= 0; i = i - 1 {
		response = append(response, response[i])
	}
	return response
}
...
// between 0 and 10000. Your function should not mutate the input array, and this will be
// tested (where applicable). Notice that the returned list will always be of odd size,
